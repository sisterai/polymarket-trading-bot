...
async function main() {
    ...
    /**
     * `--full` (with --api) used to mean "preview then redeem".
     * The current implementation redeems in a single pass; `--full` is treated as "execute" (not dry-run).
     */
    ...
    if (useAPI) {
        ...
        // NOTE: These flags are accepted for backwards compatibility, but are not implemented
        // in the current `redeemAllWinningMarketsFromAPI` helper.
        if (args.includes("--pools-within-hours")) {
            logger.warning("--pools-within-hours is currently not supported; ignoring.");
        }
        if (args.includes("--redeemable-only") || args.includes("--no-redeemable-filter")) {
            logger.warning("--redeemable-only/--no-redeemable-filter are currently not supported; ignoring.");
        }
